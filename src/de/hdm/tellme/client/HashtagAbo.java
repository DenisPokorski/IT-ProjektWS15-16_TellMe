...
public class HashtagAbo {
	...
	public Button HashtagAboLoeschenButton ()	{
		...
		HashtagAboLoeschenBtn.addClickHandler(new ClickHandler() {
			public void onClick(ClickEvent event)	{
				...
				if((auswahlIdHashtagAboLoeschen !=0) && (dropDownHashtagBereitsAbonniert.getSelectedIndex() > 0 )) {
						HashtagAboLoeschenByIds(meineId, auswahlIdHashtagAboLoeschen);
				} else {
	...
	public void HashtagAboLoeschenByIds(int NutzerId, int HashtagId) {
		asyncObj.loescheNutzeraboById(NutzerId, HashtagId, new AsyncCallback <Void>() {
		@Override
